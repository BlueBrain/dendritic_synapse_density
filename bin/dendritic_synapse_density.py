# Description:   Extracts dendritic E/I synapse densities (#synapses/um) for each neuron
# Author:        C. Pokorny
# Created:       10/02/2021
...
def create_data_splits(circuit_config, N_split):
    # Load circuit
    ...
    print(f'INFO: Created {N_split} data splits for {circuit_config}', flush=True)
...
def create_cell_table(circuit_config, gids):
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        ...
        # Process
        t_start = time.time()
        cell_ids_split = create_data_splits(circuit_config, N_split)
        cell_table = pd.concat(Parallel(n_jobs=N_jobs, verbose=100)(delayed(create_cell_table)(circuit_config, cell_ids_split[idx]) for idx in range(N_split)))
        ...
        print('Extracts dendritic synapse density (#synapses/um) for each neuron')
        print('Usage: Provide path to circuit config file')
        print('       dendritic_synapse_density PATH_TO_CONFIG <#PARALLEL_JOBS> <#DATA_SPLITS>')
